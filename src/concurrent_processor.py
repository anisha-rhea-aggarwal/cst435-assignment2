...
# ============ FILTER FUNCTIONS (TOP-LEVEL for multiprocessing) ============
...
# ============ IMAGE PROCESSING TASK (EXECUTED BY WORKERS) ============
...
if __name__ == "__main__":
    ...
    INPUT_DIR = "images"
    OUTPUT_BASE = "results/concurrent_futures"
    PROCESS_COUNTS = [2, 4, 8]

    print("=" * 70)
    print("Concurrent Futures Image Processing Pipeline")
    print("=" * 70)
    ...
    for workers in PROCESS_COUNTS:
        ...
        output_dir = os.path.join(OUTPUT_BASE, f"processes_{workers}")
    ...
    # ============ PERFORMANCE SUMMARY ============
    print("=" * 70)
    print("SUMMARY – Concurrent Futures Results")
    print("=" * 70)

    sequential_time = 8.14  

    print(f"Sequential Baseline: {sequential_time:.2f} seconds\n")
    print(f"{'Workers':<15}{'Time (s)':<15}{'Speedup':<15}{'Efficiency':<15}")
    print("-" * 60)

    for workers, parallel_time in results.items():
        speedup = sequential_time / parallel_time
        efficiency = (speedup / workers) * 100
        print(f"{workers:<15}{parallel_time:<15.2f}{speedup:<15.2f}x{efficiency:<15.1f}%")
